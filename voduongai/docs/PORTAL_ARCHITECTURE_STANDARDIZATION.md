...
Decisions this sprint surfaces as **needed but not yet made** — each blocks part of Architecture Freeze (§12):

- **ADR-004 [NEEDS DECISION] — Canonical "purchasable entity."** `courses`/`products`/`lessons` + `orders`' triple FK must be consolidated to one model, or the three must be explicitly documented as intentionally distinct (course vs. one-off product vs. lesson-level unlock) with the rendering layer (`my-products`/`account`) fixed to join all three consistently. This is the direct fix for Sprint 2's P0. **Recommend**: treat `courses` as canonical for anything sold from Premium, and either migrate `vdai-academy`'s `lessons` flow onto the same model or explicitly scope it as a separate, smaller product line.
- **ADR-005 [NEEDS DECISION] — `vdai-academy`'s relationship to `hocvienai`.** Merge, rename-and-link-from-sidebar, or formally document as two separate products (a paid lesson marketplace vs. a free CKOS-wrapper learning experience). Whatever is chosen, it must stop being an undiscoverable, unexplained duplicate.
...
- **ADR-007 [NEEDS DECISION] — Canonical Admin system.** Recommend `src/app/admin/(dashboard)/` (modern, RLS-safe via service-role + auth-checked) as canonical, with an explicit sunset plan for `admin.html`. Until decided, freeze new features from targeting `admin.html` to stop the overlap from growing.
...
- **ADR-010 [PROCESS, RECOMMEND RATIFYING] — Periodic dead-code sweep.** Sprint 3 found and removed one orphaned fake-data file (`knowledge-garden.ts`); this sprint found a second candidate (`ReflectionJournalCard.tsx`). Recommend a lightweight recurring check (could be a CI script cross-referencing `src/data/**`/`src/components/**` against actual import graphs) rather than relying on it being caught incidentally during unrelated audits.
...
**What must be resolved before Freeze can honestly be declared:**
1. ADR-004 (purchasable entity) — resolving this doesn't require finishing implementation, but does require a written decision that future Sprints build toward, since freezing architecture *around* an unresolved P0 data-integrity gap would lock in the confusion.
2. ADR-005 (`vdai-academy`) — same reasoning; freezing "Academy" as a concept while two contradictory implementations of it both exist in production is not a real freeze.
3. ADR-007 (canonical Admin) — freezing Portal architecture while two Admin apps can still silently clobber each other's writes to the same tables undermines the freeze's own stated purpose (trust, consistency).
4. A written version of ADR-002 (Companion one-directional rule) added to `THE_PORTAL_ARCHITECTURE_FREEZE.md` itself, so "Architecture v1.0" actually documents the rule this audit found being followed only by convention.

ADR-006, ADR-008, ADR-009, ADR-010, and Drift #1/#9/#11 are real but lower-severity — they do not need to block a Freeze decision, only be logged as the first items on the post-Freeze roadmap.

**Recommended next Sprint**: a decision-only session (Product Owner + technical lead) to close ADR-004/005/007 as written decisions (not full implementations), followed by a narrow implementation sprint scoped exactly to those three, then a re-issued IMP-SPR-004-B to confirm the gates above are closed before Architecture v1.0 is formally declared frozen.
