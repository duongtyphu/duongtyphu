...
| 5b | SolarGroup — nội dung theo mockup (hero, 5 chỉ số, 8 tab, Sovelmash/AERONOVA) | **Đã xác nhận là dữ liệu thật — chưa khôi phục vào code.** `SolarGroupClient.tsx` hiện đang CHỦ ĐỘNG bỏ 580K+/194+/$50/20+/40% và đổi hero (không nhắc "Duyunov") vì lúc build không kiểm chứng được nguồn — đúng thận trọng NO-FAKE-DATA, nhưng giờ đã có nguồn xác minh. Founder xác nhận qua nguồn công khai **https://solargroup.pro/vi** — đã kiểm tra chéo (WebFetch) khớp đúng: 580.000+ người/194 quốc gia, giảm tới 40% tiêu thụ năng lượng, giảm tới 30% chi phí sản xuất, hệ số tin cậy 2,5, >100.000 động cơ Slavyanka mục tiêu, >100 chuyên gia quấn dây được cấp phép, >20 bằng sáng chế, đầu tư tối thiểu $50. AERONOVA/khí cầu không có trên trang chủ nguồn này (cần tìm đúng trang con của SolarGroup nếu muốn xác minh riêng, nhưng đã khớp với dữ liệu `ecosystem_subprojects` seed trước đó) | Khôi phục 5 chỉ số + hero "Động cơ của Duyunov — Công nghệ Slavyanka" vào `SolarGroupClient.tsx`, trích nguồn `solargroup.pro/vi` làm căn cứ khi cần đối chiếu lại |
...
độc lập, không gộp cứng vào việc rút gọn trang chủ). Mục 5b (số liệu
SolarGroup) đã được Founder xác nhận là dữ liệu thật, nguồn
`solargroup.pro/vi` — không còn là điểm chặn, chỉ còn việc khôi phục vào
code ở Giai đoạn 4.
