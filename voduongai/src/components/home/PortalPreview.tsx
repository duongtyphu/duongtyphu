    ...
    <section className="mesh-light py-14 text-brand-navy md:py-20">
      <div className="mx-auto max-w-6xl px-5">
        ...
        <motion.div
          ...
          className="card-tilt mx-auto mt-10 max-w-5xl overflow-hidden rounded-[28px] border border-brand-gray-200 bg-white shadow-2xl shadow-brand-blue/10 backdrop-blur [perspective:1200px]"
        >
          <div className="flex items-center gap-3 border-b border-brand-gray-200 px-5 py-3.5">
            <span className="h-2.5 w-2.5 rounded-full bg-red-400/80" />
            <span className="h-2.5 w-2.5 rounded-full bg-yellow-400/80" />
            <span className="h-2.5 w-2.5 rounded-full bg-green-400/80" />
            <div className="ml-3 flex flex-1 items-center gap-2 rounded-full bg-brand-gray-50 px-3 py-1.5 text-xs text-brand-gray-400">
              <span>⌕</span>
          ...
          <div className="flex">
            <div className="hidden w-44 flex-shrink-0 space-y-1 border-r border-brand-gray-200 p-4 sm:block">
              {sidebar.map((item, i) => (
                <div
                  key={item}
                  className={`rounded-lg px-3 py-2 text-xs font-medium ${
                    i === 0 ? "bg-brand-gray-100 text-brand-navy" : "text-brand-gray-500"
                  }`}
                >
                  {item}
                </div>
            ...
            <div className="flex-1 p-5">
                  ...
                  <span
                    ...
                    className={`rounded-full px-3 py-1 text-[11px] font-semibold ${
                      i === 0
                        ...
                        : "border border-brand-gray-200 text-brand-gray-500"
                    }`}
                  >
                    {f}
                  </span>
              ...
              <div className="mt-5 grid grid-cols-2 gap-3 md:grid-cols-3">
                {cards.map((c) => (
                  <div
                    key={c.title}
                    className="card-shine rounded-xl border border-brand-gray-200 bg-brand-gray-50 p-4"
                  >
