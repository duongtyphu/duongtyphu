...
import { ManifestTab } from "@/components/admin/companion/tabs/ManifestTab";
import { CredentialArchitectureTab } from "@/components/admin/companion/tabs/CredentialArchitectureTab";
...
function buildTabs(aiConfigured: boolean): AdminTab[] {
  return [
    ...
    { key: "manifest", label: "Provider Manifest", content: <ManifestTab /> },
    { key: "credentials", label: "Credential Architecture", content: <CredentialArchitectureTab /> },
...
const TAB_GROUPS: AdminTabGroup[] = [
  ...
  { label: "Quản trị", keys: ["safety", "runtime", "inference", "manifest", "credentials", "test", "versions"] },
];

/**
 * 1 mục sidebar duy nhất ("Companion") chứa 10 tab CMS gốc + 4 tab Runtime
 * ("Chat Runtime" — EPIC-CS-001, "Nhà cung cấp AI (Inference)" — CS-01/
 * CS-02, "Provider Manifest" — CS-03, "Credential Architecture" — CS-04)
 * — không tạo route riêng, không lộ thuật ngữ kỹ thuật (key nội bộ tiếng
 * Anh, nhãn hiển thị tiếng Việt). Mỗi tab tự đọc dữ liệu qua
 * useCollection()/Server Action riêng của chính nó (riêng tab
 * "Credential Architecture" KHÔNG đọc gì cả — thuần hiển thị tĩnh) —
 * không cần prefetch server. `aiConfigured` là giá trị thật từ
 * isAiConfigured() (server-only, đọc biến môi trường) — page.tsx tính sẵn
 * rồi truyền xuống, Client Component không tự đọc được process.env phía
 * server.
 */
