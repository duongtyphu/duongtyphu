...
/**
 * Bản sao thứ 10 (byte-for-byte, chỉ khác import `useEditMode`) — xem
 * `src/components/portal/opportunities/EditableRegion.tsx` cho lịch sử
 * đầy đủ: `createPortal` vào `document.body` (thoát `overflow-hidden` của
 * mọi wrapper full-bleed) + `toFormValueFor`/`fromFormValueFor` (hỗ trợ
 * field `transform: "newline-list"` cho mảng string nhiều dòng như
 * `expertise`/`achievements` của Founder — tránh bug cắt sai theo dấu
 * phẩy nếu dùng `type: "tags"`).
 */
