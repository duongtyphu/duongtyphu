...
export function LearningPathGrid({ paths }: { paths: LearningPath[] }) {
  return (
    <section>
      <h2 className="text-lg font-bold text-gray-900">Lộ trình học</h2>
          ...
          <Link key={p.id} href={p.href} className="block">
            <GemCard className="h-full transition hover:-translate-y-1">
              <div className="flex items-center justify-between gap-2">
                ...
                <span className="shrink-0 rounded-full bg-gray-100 px-2 py-0.5 text-[10px] font-semibold text-gray-500">{p.level}</span>
              </div>
              <p className="mt-1.5 text-xs text-gray-500">{p.description}</p>
              <div className="mt-4">
