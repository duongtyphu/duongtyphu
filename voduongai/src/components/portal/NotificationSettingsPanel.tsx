...
export function NotificationSettingsPanel() {
  ...
  return (
    <div className="card-shine rounded-2xl border border-gray-200 bg-white/[0.04] p-5">
      <h3 className="text-sm font-bold text-gray-900">Cài đặt thông báo</h3>
      <p className="mt-1 text-xs text-gray-500">Chọn những thông báo bạn muốn nhận</p>
      <div className="mt-4 space-y-2">
        {prefs.map((p) => (
          <div key={p.id} className="flex items-center justify-between gap-3 rounded-xl border border-gray-200 bg-gray-50 p-4">
            <div>
              <p className="text-sm font-semibold text-gray-900">{p.icon} {p.title}</p>
