...
/**
 * Nhóm 3 — Live-edit Mirror. Y hệt
 * `su-menh-companion/EditableRegion.tsx` (pilot) /
 * `gem-home/EditableRegion.tsx`, chỉ đổi import `useEditMode` sang bản
 * riêng của mirror. Ngoài edit mode (Portal thật), render
 * `<>{children}</>` — zero rủi ro layout.
 */
export function EditableRegion<T extends { id: string }>({
  ...
  return (
    <Wrapper className={`relative ${as === "span" ? "inline" : ""} ${className ?? ""}`}>
      <Wrapper
        onClick={openPopover}
        className={`cursor-pointer rounded outline-dashed outline-1 outline-blue-300/70 transition hover:outline-blue-500 hover:bg-blue-50/10 ${as === "span" ? "inline" : "block"}`}
      >
        {children}
        <Pencil className="ml-1 inline-block h-3 w-3 -translate-y-0.5 text-blue-300" />
      </Wrapper>
