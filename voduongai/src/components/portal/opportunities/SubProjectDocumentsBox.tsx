...
/**
 * "Link tải tài liệu" cho DỰ ÁN CON — byte-for-byte cùng logic
 * `EcosystemDocumentsBox.tsx` (cấp hệ sinh thái chính), chỉ khác nguồn
 * Context (`useSubProjectChrome()`).
 */
export function SubProjectDocumentsBox({
  ...
  async function handleSave() {
    setSaveState("saving");
    try {
      await update(sub.id, { documents: draft });
      setSaveState("saved");
    } catch {
      setSaveState("error");
    }
