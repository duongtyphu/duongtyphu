...
export function LifeProfileCard({ lifeProfile }: { lifeProfile: LifeProfile }) {
  ...
  return (
    <div className="card-shine rounded-2xl border border-white/10 bg-white/[0.04] p-5">
      <h3 className="text-sm font-bold text-white">Điều bạn muốn Companion nhớ</h3>
        ...
        <>
          ...
          <div className="mt-4 flex flex-wrap items-center gap-3">
            ...
            <button
              ...
              onClick={handleShare}
              disabled={!dateValue || status === "saving"}
              className="rounded-lg border border-white/10 px-4 py-2 text-xs font-semibold text-white/80 transition hover:border-white/30 disabled:opacity-50"
            >
              Cập nhật
            </button>
            <button
            ...
            <button
              ...
              disabled={status === "saving"}
              className="rounded-lg border border-red-400/20 px-4 py-2 text-xs font-semibold text-red-400/80 transition hover:border-red-400/40 disabled:opacity-50"
            >
              Xoá
            </button>
          </div>
      ...
      {status === "error" && <p className="mt-2 text-xs text-red-400">Có lỗi xảy ra, vui lòng thử lại.</p>}
    </div>
  );
}
