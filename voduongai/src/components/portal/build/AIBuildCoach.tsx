import { Sparkles } from "lucide-react";
import { Button } from "@/components/portal/ui/Button";
import { GemCard } from "@/components/portal/ui/GemCard";
import type { AiBuildTip } from "@/data/portal/build-os";

/**
 * Answers: "Trong cả Hệ Kiến Tạo rộng lớn này, tôi nên bắt đầu hành động từ đâu hôm nay?"
 */
export function AIBuildCoach({ tip }: { tip: AiBuildTip }) {
  return (
    <GemCard variant="featured">
      <div className="flex items-center gap-2">
        <Sparkles className="h-4 w-4 text-[#FBBF24]" />
        <h2 className="text-sm font-bold text-white">AI Build Coach</h2>
      </div>
      <p className="mt-3 text-sm text-white/70">{tip.message}</p>
      <Button href={tip.href} variant="primary" className="mt-4">
        {tip.cta} →
      </Button>
    </GemCard>
  );
}
