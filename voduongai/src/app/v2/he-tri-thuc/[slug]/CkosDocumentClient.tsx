...
/* =============================================================================
 * `/v2/he-tri-thuc/[slug]` — trang chi tiết tài liệu CKOS, KHÔNG có mockup
 * riêng (đã xác nhận với Founder). Không "diễn giải lại thiết kế" — tái dùng
 * NGUYÊN VĂN sidebar/topbar/token màu-font của `/v2/he-tri-thuc`
 * (`CkosClient.tsx`/`he-tri-thuc.css`, tiền tố `.ckos`), chỉ khác phần nội
 * dung trung tâm (trang đọc 1 tài liệu thay vì danh sách).
 *
 * `getCkosDocument()` (`live-ckos.ts`, Bước D) đã tự khoá `body` ở tầng
 * SERVER cho tài liệu Premium khi user chưa Premium (`doc.locked=true`,
 * `doc.body=""`) — trang này chỉ hiển thị đúng những gì hàm đó trả về,
 * không tự nới quyền ở client.
 *
 * Không render markdown qua `dangerouslySetInnerHTML`/thư viện mới — cùng
 * nguyên tắc `MarkdownLite.tsx` (Companion chat) đã áp dụng cho nội dung
 * admin-authored: parse thủ công (`##`/`###` heading, `- `/số thứ tự list,
 * **đậm**), an toàn tuyệt đối, không thêm dependency.
 * ========================================================================== */
...
import type { CkosDocumentDetail } from "@/lib/portal/live-ckos";
...
/** Bôi đậm `**...**` trong 1 dòng — phần MARKDOWN INLINE duy nhất cần hỗ trợ
    (nội dung nguồn không dùng nghiêng/link/code inline). */
...
/** Parse block-level: `##`/`###` heading, `- `/`1. ` list liên tiếp gộp 1
    khối, còn lại là đoạn văn. Không dùng thư viện markdown/`dangerouslySetInnerHTML`
    — cùng nguyên tắc an toàn đã dùng cho `MarkdownLite.tsx`. */
function DocumentBody({ body }: { body: string }) {
  ...
  while (i < lines.length) {
    ...
    // Đoạn văn — gộp các dòng liên tiếp không rỗng, không phải heading/list.
...
export function CkosDocumentClient({
  doc,
  premium,
}: {
  doc: CkosDocumentDetail;
            ...
            <div className="center-col" style={{ maxWidth: 760 }}>
              ...
              <article className="card doc-article">
                <div className="doc-article-meta">
                  ...
                  {doc.accessLevel === "premium" ? <span className="doc-lock">Premium</span> : null}
                </div>

                <h1 className="doc-article-title">{doc.title}</h1>
                {doc.summary ? <p className="doc-article-summary">{doc.summary}</p> : null}
                ...
                {doc.locked ? (
                  <div className="doc-article-locked">
                    <div className="ico">
                      <DocIcon />
                    </div>
                    <h4>Nội dung dành cho thành viên Premium</h4>
                    <p>Nâng cấp Premium để đọc toàn bộ tài liệu này cùng kho tri thức CKOS đầy đủ.</p>
                    <button className="doc-article-unlock-btn" onClick={() => go("Premium.html")}>
                      Nâng cấp Premium
                    </button>
                  </div>
                ) : (
