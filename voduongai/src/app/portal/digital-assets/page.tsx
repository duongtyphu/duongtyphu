  ...
  return (
    <div className="space-y-10">
      ...
      <div className="card-shine glow-blue rounded-2xl border border-brand-blue/30 bg-brand-blue/5 p-8">
        <h1 className="text-2xl font-extrabold text-gray-900">ĐẦU TƯ CÙNG TÔI</h1>
      ...
      <div id="categories" className="space-y-4">
        <h2 className="text-sm font-bold text-gray-900">Lĩnh vực</h2>
        <div className="flex flex-wrap gap-2">
          <button
            onClick={() => setActiveCategory("all")}
            className={`rounded-full px-4 py-2 text-xs font-semibold transition ${
              activeCategory === "all" ? "bg-brand-blue text-white" : "border border-gray-200 text-gray-600 hover:text-gray-900"
            }`}
          >
            Tất cả
          </button>
          {categories.map((c) => {
            ...
            return (
              ...
              >
                <span>{c.icon}</span>
                <span>{c.name}</span>
                <span className="rounded-full bg-gray-100 px-1.5 py-0.5 text-[10px]">{count}</span>
              </button>
            );
          })}
        </div>
          ...
          {categories.map((c) => (
            <button
              ...
              className={`card-shine rounded-2xl border p-5 text-left transition hover:-translate-y-0.5 ${
                activeCategory === c.key ? "border-brand-blue/40 bg-brand-blue/5" : "border-gray-200 bg-white/[0.04]"
              }`}
            >
      ...
      <div className="flex flex-wrap items-center gap-3">
        <input
          ...
          className="w-full max-w-xs rounded-full border border-gray-200 bg-gray-50 px-4 py-2 text-sm text-gray-900 placeholder:text-gray-400 focus:border-brand-blue focus:outline-none"
        />
        <select
          value={badgeFilter}
          onChange={(e) => setBadgeFilter(e.target.value as DigitalAssetBadge | "all")}
          className="rounded-full border border-gray-200 bg-gray-50 px-4 py-2 text-xs font-semibold text-gray-600 focus:border-brand-blue focus:outline-none"
        >
      ...
      <div className="space-y-4">
        <h2 className="text-sm font-bold text-gray-900">Dự án ({filtered.length})</h2>
        {filtered.length === 0 ? (
      ...
      {featuredProjects.length > 0 && (
        <div className="space-y-4">
          <h2 className="text-sm font-bold text-gray-900">Dự án nổi bật</h2>
          <div className="grid gap-4 sm:grid-cols-2 md:grid-cols-3">
      ...
      {latestArticles.length > 0 && (
        <div className="space-y-4">
          <h2 className="text-sm font-bold text-gray-900">Bài viết mới nhất</h2>
            ...
            {latestArticles.map((a) => {
              return (
                ...
                >
                  ...
                  <h3 className="mt-2 text-sm font-bold text-gray-900">{a.title}</h3>
                  <p className="mt-1.5 line-clamp-2 text-xs text-gray-500">{a.excerpt}</p>
                  <p className="mt-3 text-[11px] text-gray-400">{a.publishedAt}</p>
                </Link>
              );
            })}
          </div>
