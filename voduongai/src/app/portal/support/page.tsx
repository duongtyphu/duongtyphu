...
export default async function SupportPage() {
  ...
  return (
    <div className="space-y-8">
      <div>
        <h1 className="text-2xl font-extrabold text-gray-900">Hỗ trợ</h1>
      ...
      {!email ? (
        ...
        <>
          ...
          <div>
            <h2 className="text-lg font-bold text-gray-900">Yêu cầu của bạn</h2>
            {tickets.length === 0 ? (
              <p className="mt-3 text-sm text-gray-500">Bạn chưa gửi yêu cầu hỗ trợ nào.</p>
            ) : (
              <div className="mt-4 space-y-3">
                {tickets.map((t) => (
                  <div key={t.id} className="card-shine rounded-2xl border border-gray-200 bg-white/[0.04] p-5">
                    ...
                    {t.reply && (
                      <div className="mt-3 rounded-xl border border-brand-blue/20 bg-brand-blue/5 p-3">
                        ...
                        <p className="mt-1 text-sm text-gray-700">{t.reply}</p>
                      </div>
                    )}
                  </div>
                ))}
