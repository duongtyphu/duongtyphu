...
| **Input** | Input trong `story/ReflectionJournalCard.tsx`, `account/LifeProfileCard.tsx` | `rounded-xl border border-gray-200 bg-gray-50`, focus ring `focus:ring-1 focus:ring-blue-400/50`. Chưa có component Input dùng chung — **cần trích xuất thành `ui/Input.tsx`** (đề xuất bước tiếp theo). |
...
| **Reflection Card** | `story/ReflectionJournalCard.tsx`, Reflection section (`hanh-trinh-cua-toi/page.tsx`) | Câu hỏi mở + khoảng trống để trả lời/textarea, giọng văn mời gọi không ép buộc. |
