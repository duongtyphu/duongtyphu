...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function handleLeadSubmit(form, result) {
    ...
    // Nếu chưa cấu hình endpoint, không giả vờ gửi thành công
    if (!VDAI_CONFIG.form.isConnected || !VDAI_CONFIG.form.leadEndpoint) {
      statusEl.className = 'form-status error show';
      statusEl.innerHTML = errorIcon() +
        '<span>Hệ thống nhận dữ liệu chưa được kết nối. Vui lòng liên hệ trực tiếp qua Zalo để nhận bản đồ Affiliate AI.</span>';
      trackEvent('lead_error', { reason: 'endpoint_not_configured' });
      return;
    }
    ...
    fetch(VDAI_CONFIG.form.leadEndpoint, {
      ...
      .then(() => {
        ...
        statusEl.innerHTML = successIcon() + '<span>Đã gửi thành công! Chúng tôi sẽ liên hệ với bạn sớm nhất.</span>';
        trackEvent('lead_success', { track: result.ctaTrack });
        submitBtn.textContent = 'Đã gửi ✓';

        // Chuyển đến trang cảm ơn sau 1.5s (nếu trang đang ở root)
        setTimeout(() => {
